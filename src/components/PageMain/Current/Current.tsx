    ...
    >
      <div className="current-container">
        ...
        <div className="weather-container">
          ...
          <h3 className="text-2xl text-slate-950">
            {dayjs.unix(weather.dt).format("MMM-D-YYYY")}
          </h3>
          <h3 className="text-2xl text-slate-950">{weather.weather[0].main}</h3>
          {weather.weather[0].main === "Mist" && (
            <img className="w-24" src="/public/icons/misty.svg" />
          )}
          {weather.weather[0].main === "Clear" && (
            <img className="w-24" src="/public/icons/sunny.svg" />
          )}
          {weather.weather[0].main === "Clouds" && (
            <img className="w-24" src="/public/icons/cloudy.svg" />
          )}
          {weather.weather[0].main === "Rain" && (
            <img className="w-24" src="/public/icons/rainy.svg" />
          )}
          {weather.weather[0].main === "Thunderstorms" && (
            <img className="w-24" src="/public/icons/stormy.svg" />
          )}
          {weather.weather[0].main === "Snow" && (
            <img className="w-24" src="/public/icons/snowy.svg" />
          )}
          {weather.weather[0].main === "Haze" && (
            <img className="w-24" src="/public/icons/hazy.svg" />
          )}
          <h3 className="text-2xl text-slate-950">
