...
const ForecastDayOne = ({ forecast }: any) => {
  return (
    <section className="bg-gradient-to-b from-blue-300 to-blue-200 p-4 border-2 text-md text-slate-950 border-blue-950 rounded-2xl w-full">
      ...
      <h3 className="text-2xl text-slate-950">
          {dayjs.unix(forecast.list[0].dt).add(1, "day").startOf("day").format("dddd")}
        </h3>
        <h3 className="text-xl text-slate-950">
          {dayjs.unix(forecast.list[0].dt).add(1, "day").startOf("day").format("MMM-D-YYYY")}
        </h3>
        <h3 className="text-xl">{forecast.list[0].weather[0].main}</h3>
        {forecast.list[0].weather[0].main === "Mist" && (
        ...
        {forecast.list[0].weather[0].main === "Thunderstorms" && (
          <img className="w-12" src="/public/icons/stormy.svg" />
        )}
        {forecast.list[0].weather[0].main === "Snow" && (
          <img className="w-12" src="/public/icons/snowy.svg" />
        )}
        {forecast.list[0].weather[0].main === "Haze" && (
          <img className="w-12" src="/public/icons/hazy.svg" />
        )}
        <h3>{`Temp: ${forecast.list[0].main.temp.toFixed(0)}° F`}</h3>
        <h3>{`Feels: ${forecast.list[0].main.feels_like.toFixed(0)}° F`}</h3>
        <h3>{`Humidity: ${forecast.list[0].main.humidity}%`}</h3>
        <h3>{`Wind: ${forecast.list[0].wind.speed} mph`}</h3>
      </div>
    </section>
  );
};

export default ForecastDayOne;
